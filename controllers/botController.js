...
// --- Fallback parser for AI ---
function fallbackParseOrderIntent (message) {
  console.log('🔄 Using fallback order parser...')
  const parts = message.split(/,| and /i).map(p => p.trim()).filter(Boolean)
  const items = parts.map(part => parseItemPartFallback(part)).filter(Boolean)
  return { items }
}
...
export const handleIncomingMessage = async ({ from, text, profile, messageId }, res) => {
  try {
    ...
    if (
      user.isOnboarded &&
      ['hi', 'hello', 'hey', 'good morning', 'good afternoon', 'good evening'].includes(normalized)
    ) {
      return replyAndExit(
        from,
        `👋 Hi ${user.fullName || user.whatsappName}! You can place an order anytime. Just tell me what you want washed 🙂`,
        res,
        messageId
      )
    }
    ...
    if (!user.isOnboarded) {
      ...
      if (!step) {
        user.conversationState = { step: 'awaiting_name' }
        await user.save()
        return replyAndExit(from, '👋 Welcome! Please tell me your *full name* to get started.', res, messageId)
      }
      if (step === 'awaiting_name') {
        user.fullName = text.trim() || user.whatsappName
        user.conversationState = { step: 'awaiting_address' }
        await user.save()
        return replyAndExit(from, `📍 Thanks ${user.fullName}! Now send me your *address*.`, res, messageId)
      }
      if (step === 'awaiting_address') {
        user.address = text.trim()
        user.conversationState = { step: 'awaiting_preferences' }
    ...
    if (user.conversationState?.step) {
      ...
      if (state.step === 'awaiting_items') {
        ...
        user.conversationState = state
        await user.save()
        return handleIncomingMessage({ from, text: 'continue order', profile, messageId: fakeMessageId }, null)
      }
